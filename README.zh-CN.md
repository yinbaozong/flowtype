...
2. 同一个 `v0.5.2` Release 中同时提供两种文件。
3. 免安装版：下载 `FlowType.Portable.0.5.2.zip`，完整解压到固定文件夹，再运行 `FlowType.exe`。
4. 安装版：下载 `FlowType.Setup.0.5.2.exe`，运行后选择安装位置。
...
安装包输出到 `release/FlowType Setup 0.5.2.exe`，免安装包输出到 `release/FlowType Portable 0.5.2.zip`。
