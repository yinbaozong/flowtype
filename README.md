...
- Configurable one-press undo shortcut that removes the most recent FlowType paste as one edit operation.
...
2. The same `v0.5.2` release contains both downloads: `FlowType.Portable.0.5.2.zip` for portable use and `FlowType.Setup.0.5.2.exe` for normal installation.
...
The portable archive is written to `release/FlowType Portable 0.5.2.zip`. It keeps the original Electron executable byte-for-byte and packages FlowType in the adjacent `resources/app.asar` file. The installer is written to `release/FlowType Setup 0.5.2.exe`.
